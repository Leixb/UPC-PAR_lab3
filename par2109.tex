...
This serialization in the graphical version is caused in the lines 120-130 of the code due to a data race problem that implies full dependence between all tasks. This section can be protected with the \texttt{\#pragma omp critical} clause as shown in the listing~\ref{listing:mandel-omp-critical}. % is this ok?
...
% TODO: Reason which one of the two task granularities would be more appropriate to apply to implement a parallel version of the Mandelbrot code.
\textbf{ATENCION FALTA HACER!}
Esto ni idea... yo diria que el POINT seria lo mejor pero no hay infinitos threads por lo tanto teniendo los threads que tenemos quizas es mejor ROW no?
